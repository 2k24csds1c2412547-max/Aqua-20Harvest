...
const generateMockWeatherData = (latitude: number, longitude: number): WeatherData => {
  ...
  const monthlyRainfall = Array.from({ length: 12 }, (_, index) => {
    ...
    if (index >= 5 && index <= 8) {
      return Math.round((baseRainfall * 0.15) + (Math.random() * baseRainfall * 0.25));
    }
...
export const getIndianStateFromCoordinates = (latitude: number, longitude: number): string => {
  // Simplified state detection based on coordinates
  // In production, you'd use a more sophisticated geocoding service
  
  if (latitude >= 23.5 && latitude <= 37 && longitude >= 68 && longitude <= 97) {
    // Northern states
    if (latitude >= 30) {
